...
import { advanceOrder, emptyOrders, flagOrderException, ORDER_STORAGE_KEY, purgeExpiredOrderData, receiveOrder, resolveOrderException, type ExceptionCategory, type ExceptionResolution, type OrderState, type OrderStatus } from '@/lib/orders';
...
export function OrdersWorkspace(){
 ...
 return <div className="space-y-6">
  ...
  {state.orders.length>0&&<button className="ddf-button secondary" onClick={()=>act(()=>purgeExpiredOrderData(state,new Date().toISOString()),'Política de retenção aplicada.')}>Aplicar retenção agora</button>}
