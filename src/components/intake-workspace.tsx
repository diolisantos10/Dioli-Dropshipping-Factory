...
export function IntakeWorkspace({ mode }: { mode: 'intake' | 'triage' | 'audit' | 'overview' }) {
  ...
  return <div className="space-y-6">
    ...
    <p className="surface p-4 text-sm">Demonstração local. Os registros ficam neste navegador; não são compartilhados entre dispositivos. As aprovações não iniciam processamento externo. Autenticação e auditoria de produção ainda serão implementadas.</p>
    ...
    {!ready ? <p>Cadastro indisponível até concluir a leitura dos dados.</p> : <>
      ...
      {mode === 'intake' && <form className="surface grid gap-4 p-6" onSubmit={event => {
        ...
        try { persist(addCandidate(state, { name: String(data.get('name')), url: String(data.get('url')), notes: String(data.get('notes')) }, crypto.randomUUID(), new Date().toISOString())); form.reset(); setMessage('Candidato cadastrado. Nenhuma produção iniciada.'); } catch (e) { setError(e instanceof Error ? e.message : 'Não foi possível salvar.'); }
      }}>
      ...
      {mode === 'overview' ? null : mode !== 'audit' ? <>
        ...
        {candidates.map(c => <article key={c.id} className="surface space-y-3 p-6">
          ...
          <p className="break-all text-sm text-gray-600">{c.url}</p><p className="whitespace-pre-wrap">{c.notes || 'Sem observações.'}</p>
