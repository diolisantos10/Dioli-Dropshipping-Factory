...
export function ServerStateBridge(){const[state,setState]=useState<'loading'|'online'|'offline'>('loading');useEffect(()=>{let active=true;
  const pull=()=>Promise.all(stores.map(async([namespace,key,event])=>{const response=await fetch(`/api/state/${namespace}`,{cache:'no-store'});if(!response.ok)throw new Error();const data=await response.json();const next=data.payload?JSON.stringify(data.payload):null;if(next===null)localStorage.removeItem(key);else if(localStorage.getItem(key)!==next)localStorage.setItem(key,next);else return;window.dispatchEvent(new Event(event));})).then(()=>active&&setState('online')).catch(()=>active&&setState('offline'));
